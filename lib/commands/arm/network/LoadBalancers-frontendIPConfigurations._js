...
exports.init = function (cli) {
  ...
  frontendIPConfigurations.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      cli.interaction.formatOutput(result.frontendIPConfigurations[generatorUtils.findIndexByKeyValue(result.frontendIPConfigurations, 'name', name)], generatorUtils.traverse);
    });
  ...
  frontendIPConfigurations.command('set [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      cli.interaction.formatOutput(result.frontendIPConfigurations[index], generatorUtils.traverse);
    });
  ...
  frontendIPConfigurations.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(result.frontendIPConfigurations, {name: name});
      if (index === -1) {
        throw new Error(util.format($('frontend ip configurations "%s" not found in the lbName "%s"'), name, result.name));
      }
  ...
  frontendIPConfigurations.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.frontendIPConfigurations, {name: name});
      if(!childResult) {
        cli.output.warn(util.format($('frontend ip configurations with name "%s" not found in the lbName "%s"'), name, result.name));
      }
