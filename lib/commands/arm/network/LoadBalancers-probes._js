...
exports.init = function (cli) {
  ...
  probes.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(options.protocol) {
        ...
        if(options.protocol.toLowerCase() == 'TCP'.toLowerCase()) {
          delete parameters.probes[index].requestPath;
        }
      } else if(useDefaults) {
        parameters.probes[index].protocol = 'TCP';
      }
      if(options.port) {
        parameters.probes[index].port = parseInt(options.port, 10);
      } else if(useDefaults) {
        parameters.probes[index].port = parseInt('80', 10);
      }
      if(options.interval) {
      ...
      cli.interaction.formatOutput(result.probes[generatorUtils.findIndexByKeyValue(result.probes, 'name', name)], generatorUtils.traverse);
    });
  ...
  probes.command('set [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      cli.interaction.formatOutput(result.probes[index], generatorUtils.traverse);
    });
  ...
  probes.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(result.probes, {name: name});
      if (index === -1) {
        throw new Error(util.format($('probes "%s" not found in the lbName "%s"'), name, result.name));
      }
  ...
  probes.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.probes, {name: name});
      if(!childResult) {
        cli.output.warn(util.format($('probes with name "%s" not found in the lbName "%s"'), name, result.name));
      }
