var resourceUtils = require('../commands/arm/resource/resourceUtils');
...
exports.showTableRow = function(row, obj) {
  ...
  var maxFields = 12;
  try {
    resInfo = resourceUtils.getResourceInformation(obj.id);
  } catch(e) {
    // Skip failure in case id is not supposed to contain required data
    if(e.message != 'invalid resource id from server')
    {
      throw e;
    }
  }
  finally {
    if(resInfo && resInfo.resourceGroup) {
  ...
  for (var i in obj) {
    var item = obj[i];
    if (typeof(item) != 'object') {
      if(i != 'id' && i != 'resourceId' && i != 'resourceGuid' && i != 'etag' && i != 'type') {
      ...
      if (item !== null) {
        if (i === 'tags') {
          var tagsObj = JSON.stringify(item);
          row.cell($('Tags'), tagsObj);
          continue;
        }
        if (i === 'name') {
          if(item.localizedValue) {
            row.cell($('Name'), item.localizedValue);
            continue;
          }
        }
        if(item instanceof Array && fieldsCount < maxFields) {
